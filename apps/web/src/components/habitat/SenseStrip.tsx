...
const displayUnits: Record<string, string> = { "[ft_i]3/s": "ft³/s", "[ft_i]": "ft", "[in_i]": "in", "ug/m3": "µg/m³", "m3/s": "m³/s", "Cel": "°C" };
...
export function SenseStrip({ snapshot }: { snapshot: HealthSnapshot | null }) {
  ...
  return <ul className="habitat-sense-strip" aria-label="Ecological senses summary">{snapshot.needs.map((need) => {
    ...
    return <li key={`${need.need}-${need.property}`} data-stale={need.stale}>
      <span>{label}</span><strong>{need.stale ? "Stale" : need.value === null ? "Unknown" : `${Number(need.value.toPrecision(4))} ${displayUnits[need.unit ?? ""] ?? need.unit ?? ""}`}</strong>
