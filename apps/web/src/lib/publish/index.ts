...
export function getPublisher(): Publisher {
  ...
  if (process.env.BLOB_READ_WRITE_TOKEN) return (cached = new BlobPublisher());
  ...
  if (env.NODE_ENV === "production") {
    throw new Error("no publisher configured: set R2 credentials or BLOB_READ_WRITE_TOKEN");
  }
