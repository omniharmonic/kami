...
export async function loadStatus(
  ...
  if (text === null && process.env.BLOB_READ_WRITE_TOKEN) {
    try {
      // No stale private publication can escape when publication is withdrawn.
      ...
      const db = getDb();
      if (!db) return null;
      const [released] = await db.select({ id: schema.entities.id }).from(schema.entities).where(and(eq(schema.entities.slug, slug), isNotNull(schema.entities.publishedAt), isNull(schema.entities.retiredAt))).limit(1);
