...
| Phase 3 grant/retro rounds and coalitions | Roadmap / missing dedicated round surfaces | No dedicated grant-round lifecycle was found in current entity routes. Requires explicit round model, budgets, eligibility, applications, review and payout linkages before presenting a live round. Coalition shared treasuries are future scope and must not be represented as deployed. |
...
7. **Grant rounds and coalition treasuries.** Retro rounds appear in PRD phase 3, while shared coalition treasuries are a later user-requested roadmap addition. There is no complete dedicated round or coalition lifecycle in the current routes. They require budget/eligibility/application/review models and treasury linkage, not a dashboard label. Keep them marked future work until implemented and exercised.
