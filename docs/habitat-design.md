# Living habitat interface

The being's home is the main visual object. The field journal is a translucent instrument carried into that home, with a compact identity card and quiet state phrase above it. Landscape and physical sprite rendering live in the world module; the habitat UI never draws a second large character.
...
Keyboard users can move across tabs with arrow keys, Home and End. Panels are server rendered and preserve their state when switched. Small screens scroll the dock horizontally, retain two columns of senses, and keep a scene window above the journal. Reduced motion remains governed by the world renderer.

Public/private entity access stays in the existing visible-dashboard loader. Dedicated chat now uses the same private snapshot loader as the dashboard, with unknown runtime treated as offline instead of online.
