...
| 6 | `01` §7.5, `02` §7.6 | Refunds are named as a donor promise but no refund path is specified. `docs/donors.md` currently describes a steward performing one by hand. Making it a button needs a policy decision first: who may authorise a refund, and whether a donor can request one without signing in. Flagged for the owner, not implemented. | WP11 |
