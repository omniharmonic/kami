...
| §6.1 "Dashboard pages render from static JSON published nightly" | `apps/web/src/lib/jobs/needs.ts` + `apps/web/src/lib/publish/r2.ts`, hourly (`infra/vercel.json` `0 * * * *`) | `apps/web/src/lib/publish/__tests__/publish.test.ts` | **implemented** — hourly, not nightly; recorded as a deliberate deviation in ADR-E14 and arch §15 #2 |
