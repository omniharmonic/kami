...
| 74 | OpenRouter's exact contract for the gate: base URL `https://openrouter.ai/api` (so `/v1/chat/completions` resolves), the `HTTP-Referer` / `X-Title` attribution header names, the exact model slug (`qwen/qwen3.5-9b-instruct` is a guess), and that it returns a well-formed `tool_calls` array, streams, and reports `usage`. The sandbox cannot reach it; `python -m entity_gate.check_upstream --config gate.yaml` answers all four in one run and is the intended way to settle this row | WP-gate (hosted upstream) | open | `apps/gate/README.md` "Pointing the gate at a hosted API" |
| 75 | LM Studio as the upstream: the loopback port (1234), the model id `GET /v1/models` reports, and whether its tool-call output survives the gate's parser at a 64k context. Small local builds are exactly where check (b) fails | WP-gate (hosted upstream) | open | `apps/gate/README.md` LM Studio example |
