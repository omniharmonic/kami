...
source=home/'.hermes/profiles/beings-earth'
config=yaml.safe_load((source/'config.yaml').read_text())
config['model']={'default':'beings-guarded','provider':'custom','base_url':'http://127.0.0.1:8001/p/boulder-creek/v1','context_length':128000}
...
for line in (home/'.kami/credentials/boulder-creek.env').read_text().splitlines():
 if '=' in line and not line.startswith('#'):
  k,v=line.removeprefix('export ').split('=',1)
  if k=='PLATFORM_MCP_TOKEN':settings[k]=v.strip().strip('\"').strip("'")
settings.update({'GATE_ADMIN_SECRET':secret,'KAMI_PLATFORM_TOKEN':secret,'KAMI_ENTITY_SLUG':'boulder-creek','KAMI_HERMES_PROFILE_DIR':str(profile),'KAMI_CODEX_MODEL':'gpt-5.6-luna'})
fd=os.open(settings_path,os.O_WRONLY|os.O_CREAT|os.O_TRUNC,0o600)
with os.fdopen(fd,'w') as f:json.dump(settings,f)
os.chmod(settings_path,0o600)
gate={'upstream_url':'http://127.0.0.1:8002','upstream_api_key_env':'KAMI_CODEX_BRIDGE_KEY','upstream_model':'gpt-5.6-luna','paused':['boulder-creek'],'events_dir':str(private/'events'),'provenance':{'placement':'hosted','provider':'OpenAI Codex via guarded Hermes','model':'gpt-5.6-luna','slug':'boulder-creek'},'platform':{'base_url':'https://beings.earth','pause_set_url':'https://beings.earth/api/gate/pause-set','token_env':'KAMI_PLATFORM_TOKEN','fail_closed':True,'poll_seconds':15}}
...
print('Boulder Creek remains explicitly paused in gate configuration.')
